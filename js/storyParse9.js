...
function parseXML(xml) {
	...
	credits.push("<p><a href='http://people.rit.edu/~mlm3743/web-portfolio/portfolio/project_pages/digital_storytelling.php' target='_blank'>Digital storytelling</a> created by Mike Maether</p>");
	credits.push("<p><a href='http://johnpolacek.github.io/superscrollorama/' title='SUPERSCROLLORAMA website' target='_blank'>SUPERSCROLLORAMA</a> by John Polacek</p>");
...
function scrollScript(scroll_id, scrollElement){
	...
	$(scrollElement).each(function(){
		...
		if (scrollAnimationName == "fly"){
			...
			if($(this).find('fly_downwards').length){
				var bottomText = $(this).find('fly').find('fly_downwards').attr('in_pixels');
				bottomText = keepPosAndNeg(bottomText);
				bottomText = "bottom: '" + bottomText + "px'";
				directionsArray.push(bottomText);
			}
			if($(this).find('fly_upwards').length){
				var topText = $(this).find('fly').find('fly_upwards').attr('in_pixels');
				topText = keepPosAndNeg(topText);
				topText = "top: '" + topText + "px'";
				directionsArray.push(topText);
			}
			if($(this).find('fly_to_the_left').length){
				var leftText = $(this).find('fly').find('fly_to_the_left').attr('in_pixels');
				leftText = keepPosAndNeg(leftText);
				leftText = "left: '" + leftText + "px'";
				directionsArray.push(leftText);
			}
			if($(this).find('fly_to_the_right').length){
				var rightText = $(this).find('fly').find('fly_to_the_right').attr('in_pixels');
